...
- Geometries are *sticky*. This means they aren't dropped implicitly
...
1. Create some simple feature geometries (`sfg`) of objects you know "by hand". Create at least one `POINT`, one `LINESTRING` and one `POLYGON` geometry. You can capture the coordinates of the nodes from [map.geo.admin.ch](https://map.geo.admin.ch/) if these are is Switzerland and [openstreetmap.org](https://www.openstreetmap.org) (or similar) if they aren't
